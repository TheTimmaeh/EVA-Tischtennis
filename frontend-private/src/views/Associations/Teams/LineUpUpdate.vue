    ...
    setup(){
      ...
      api(`/associations/${route.params.associationId}/teams/${route.params.teamId}/members`).then((res) => res.data).then((res) => {
        ...
        rows.value = [
          ...
          { name: 'E2', text: 'Spieler 2 -  E2:', field: 'search', apiPath: '/persons', displayFormat: '{{ name }} {{ surname }}', lookupRow: ['name', 'surname'], returnPath: 'id', validate: { required: true }, value: res.data.find((d) => d.position == 'E2')?.id },
          { name: 'E3', text: 'Spieler 3 -  E3:', field: 'search', apiPath: '/persons', displayFormat: '{{ name }} {{ surname }}', lookupRow: ['name', 'surname'], returnPath: 'id', validate: { required: true }, value: res.data.find((d) => d.position == 'E3')?.id },
          { name: 'D1', text: 'Spieler 4 -  D1:', field: 'search', apiPath: '/persons', displayFormat: '{{ name }} {{ surname }}', lookupRow: ['name', 'surname'], returnPath: 'id', validate: { required: true }, value: res.data.find((d) => d.position == 'D1')?.id },
          { name: 'D2', text: 'Spieler 5 -  D2:', field: 'search', apiPath: '/persons', displayFormat: '{{ name }} {{ surname }}', lookupRow: ['name', 'surname'], returnPath: 'id', validate: { required: true }, value: res.data.find((d) => d.position == 'D2')?.id },
         ]
      })
