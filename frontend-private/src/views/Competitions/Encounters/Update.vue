...
<script>
  ...
  export default {
    ...
    setup(){
      ...
      api(`/encounters/${route.params.encounterId}`).then((res) => res.data).then((res) => {
        if(!res.success){
        ...
        rows.value = [
          ...
          { name: 'gamestage', text: 'Spielstude:', field: 'gamestageSelect', validate: { min: 2, max: 2, required: true }, value:res.data.game_stage }, 
      ]
      })
      ...
      const submit = (data) => {
        ...
        api({ method: 'POST', path: `/encounters/${route.params.encounterId}`, data }).then((res) => {
          if(!res.data){
